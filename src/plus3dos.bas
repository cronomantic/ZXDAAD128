...
END SUB
/'
FUNCTION Plus3DosSavegame(fname as uInteger, addr AS uInteger, size AS uInteger) AS uByte
...
ASM
PROC
    LOCAL readPlus3, endPlus3, closePlus3, abandonPlus3, errorPlus3
  
    LD L, (IX+4)  ; Filename
    LD H, (IX+5)
    PUSH IX
    LD BC, $0003  ; B = FIlenumber, C = access Mode
    LD DE, $0204  ; D = Create action, E = open action
...
readPlus3:
    ...
    LD D, (IX+9)
    PUSH IX
    LD BC, $0000    ; B = Filenumber, C = Bank
    CALL DOS_WRITE
    JR C, closePlus3
    INC A         ;ErrorNo + 1
    PUSH AF
...
FUNCTION Plus3DosLoadgame(fname as uInteger, addr AS uInteger, size AS uInteger) AS uByte
...
PROC
    ...
    PUSH IX
    LD BC, $0003  ; B = FIlenumber, C = access Mode
    LD DE, $0002  ; D = Create action, E = open action
    CALL DOS_OPEN ; Not carry -> Error
    JR C, readPlus3
    INC A         ;ErrorNo + 1
    JR errorPlus3
...
FUNCTION Plus3DosLoadXImage(fname AS uInteger) AS uByte
